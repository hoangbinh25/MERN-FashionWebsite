...
// Xóa mảng Products mock, sẽ lấy từ API
...
export default function Product() {
    ...
    // Function load danh sách sản phẩm
    const loadProducts = async (page = 1) => {
        setLoading(true);
        try {
            const res = await getAllProducts({ page, limit });
            // Lọc chỉ lấy sản phẩm isActive === true
            const filtered = (res.data || []).filter(item => item.isActive);
            const mapped = filtered.map(item => ({
                id: item._id,
                img: item.image?.[0] || "https://via.placeholder.com/350x350?text=No+Image",
